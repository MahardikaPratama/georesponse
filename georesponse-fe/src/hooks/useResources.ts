 ...
 * Description  : Queries the resource list (GET /api/v1/resources), using
 *                the resourceKeys factory for its cache key. Callable from
 *                multiple components (list, map) — TanStack Query shares
 *                one cache entry per identical filters, so this never
 *                issues duplicate requests, the same pattern useCurrentUser
