 ...
 * Description  : The MapAdapter boundary (the Map Adapter pattern):
 *                everything a map implementation must support, expressed
 *                without any MapLibre type. Only MapAdapter.ts (and other
 *                files in this directory) may import maplibre-gl;
 *                ResourceMap.tsx depends on this interface only.
 *
 * Changelog:
 * - 1.0.0 (2026-09-19): Initial creation.
 * - 1.1.0 (2026-09-19): Added MapMarker.color, so a marker can be painted
 *                        per Resource.status without this file or
 *                        MapAdapter.ts needing to know the Resource
 *                        domain type or its status enum — the caller
 *                        resolves the color, the adapter just paints it.
 ...
 *                        callback for create ("Placing a resource by
 *                        clicking the map ... going through the map
 *                        adapter's click callback"), fired only when the
 *                        double-click didn't land on an existing
 *                        marker/hotspot.
 */
