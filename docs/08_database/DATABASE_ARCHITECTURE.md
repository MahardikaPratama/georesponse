...
- **One pool per process.** The pool is created once at application startup
  ...
  ad hoc connections.
- **Bounded pool size.** The pool is created from the `DATABASE_URL`
  connection string alone (`pgxpool.New`), so sizing follows `pgxpool`'s
  defaults and is tuned, when needed, through the `pool_max_conns` /
  `pool_min_conns` query parameters of that URL rather than hard-coded
  values.
...
- The `resources.location` column (and `previous_location` /
  `new_location` on `resource_location_history`) is stored as
  `geography(Point, 4326)` (SRID 4326 = WGS 84, the coordinate system used
  by `latitude` / `longitude` in `DATA_CONTRACT.md`).
...
no translation layer. New identifiers are generated by the backend
(`internal/platform/idgen`, a random version-4 UUID string; seed data uses
readable ids such as `resource-001`) before insert; the database does not
silently generate ids the application layer is unaware of. This keeps `id` generation a single, testable concern in the
...
resource are retained (see `DATABASE_SCHEMA.md` sections 7 and 8 —
history/audit tables use `ON DELETE SET NULL`, not cascade, when a resource
is removed; migration `0006` corrected the history tables to this).
